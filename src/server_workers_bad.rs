...
impl Server {
    pub async fn new(
        ...
        Ok(Self {
            credentials,
            listen_port: u16::from_str(&listen_port)?,
    ...
    )  -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    {
        ...
        udp_socket.send_to(&bytes, dst_addr)?;
        Ok(())
    }

    async fn tun_read_worker(
        self: Arc<Self>,
        tun_device: Arc<AsyncDevice>,
        mut udp_socket: Arc<CustomUdpSocket>
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        loop {
            match tun_device.recv_multiple(&mut original_buffer, &mut bufs, &mut sizes, 0).await {
                Ok(read_count) => {
                    ...
                    if send_count > 0 {
                        udp_socket = tokio::task::spawn_blocking(move || {
                            udp_socket.write_batch(&mut send_buffers[..send_count]).unwrap();
                            udp_socket
                        }).await.unwrap();
                    }
                },
    ...
    //                 if send_count > 0 {
    //                     self.udpsocket.write_batch(&mut send_buffers[..send_count])?;
    //                 }
    //             },
    //             Err(e) => {
    //                 eprintln!("{e}")
    ...
    async fn handle_handshake(self: &Arc<Self>, udp_socket: &CustomUdpSocket, client_dhkey_pkt: &DHKeyPacket, src_addr: &SocketAddr) 
    -> Result<(), Box<dyn std::error::Error + Send + Sync>> 
    {
        ...
        Server::sock_write(udp_socket, dh_key_pkt_response, *src_addr).await?;
    ...
    async fn handle_auth_packet(self: &Arc<Self>, udp_socket: &CustomUdpSocket, enc_auth_data: &EncryptedMessage, src_addr: &SocketAddr) 
    -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    {
                    ...
                    Ok(auth_data) => {
                        ...
                        match self.check_authdata(auth_data) {
                            Ok(_) => {
                                ...
                                Server::sock_write(udp_socket, tun_set_pkt, *src_addr).await?;
    ...
    // async fn handle_packet(self: &Arc<Self>, udp_socket: &CustomUdpSocket, pkt: &PacketType, src_addr: &SocketAddr) 
    // -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    // {
    //     match pkt {
    //         PacketType::Handshake(dhkey_packet) => {
    //             self.handle_handshake(udp_socket, dhkey_packet, src_addr).await?;
    //         },
    //         PacketType::AuthPacket(enc_auth_data) => {
    //             self.handle_auth_packet(udp_socket, enc_auth_data, src_addr).await?;
    //         },
    ...
    async fn packet_batch_handle_worker(
        self: &Arc<Self>, 
        tun_device: Arc<AsyncDevice>,
        udp_socket: &CustomUdpSocket,
        mut packet_rx: mpsc::Receiver<(PacketType, SocketAddr)>
    ) 
    -> Result<(), Box<dyn std::error::Error + Send + Sync>>
    {
        let mut gro_table = GROTable::default();
        ...
        let mut batch_buffer = Vec::<(PacketType, SocketAddr)>::with_capacity(IDEAL_BATCH_SIZE);
        loop {
            
            let recv_count = packet_rx.recv_many(&mut batch_buffer, IDEAL_BATCH_SIZE).await;
            if recv_count == 0 {
                println!("Packet batch handle worker channel closed");
                break;
            }

            let mut send_count = 0;
            for i in 0..recv_count {
                let (pkt_type, src_addr) = &batch_buffer[i];
                match pkt_type {
                    PacketType::Handshake(dhkey_packet) => {
                        if let Err(e) = self.handle_handshake(udp_socket, dhkey_packet, src_addr).await {
                            eprintln!("Handshake error {e}");
                        }
                    },
                    PacketType::AuthPacket(enc_auth_data) => {
                        if let Err(e) = self.handle_auth_packet(udp_socket, enc_auth_data, src_addr).await {
                            eprintln!("Auth error {e}");
                        }
                    },
                    PacketType::EncryptedPkt(encrypted_message) => {
            ...
            if send_count > 0 {
                tun_device.send_multiple(&mut gro_table,  &mut bufs[..send_count], VIRTIO_NET_HDR_LEN).await?;
                bufs.clear();
            }
            batch_buffer.clear();
        }
    ...
    fn sock_read_worker(
        self: &Arc<Self>,
        udp_socket: &CustomUdpSocket,
        packet_tx: mpsc::Sender<(PacketType, SocketAddr)>
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        //prepare recvmmsg
        let mut msghdrs: [mmsghdr; MAX_DATAGRAMS] = unsafe { std::mem::zeroed() };
        let mut iovs: [iovec; MAX_DATAGRAMS] = unsafe { std::mem::zeroed() };
        ...
        'outer: loop {
            //let mut guard = self.udpsocket.readable().await?;
            
            let num_received = match udp_socket.read_batch(&mut msghdrs) {
                Ok(num_received) => {
                    num_received
                },
                Err(e) => {
            ...
            for i in 0..num_received {
                ...
                if len > 0 {
                    ...
                    let send_result = tokio::runtime::Handle::current()
                        .block_on(packet_tx.send((pkt, src_addr)));
    ...
    pub async fn run(self : &Arc<Self>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for worker_id in 0..max_threads {
            ...
            let tun_device = Arc::new(create_tun(&self.tun_ip.to_string(), &self.tun_netmask.to_string())?);
            let udp_socket = Arc::new(CustomUdpSocket::new(self.listen_port)?);

            let (packet_tx, packet_rx) = mpsc::channel::<(PacketType, SocketAddr)>(100000);
            set.spawn({
                let me = self.clone();
                let tun_device = tun_device.clone();
                let udp_socket = udp_socket.clone();
                async move {
                    me.tun_read_worker(tun_device.clone(), udp_socket).await
                }
            });
            ...
            set.spawn({
                ...
                let udp_socket = udp_socket.clone();
                async move {
                    me.packet_batch_handle_worker(tun_device, &udp_socket, packet_rx).await
                }
            });
            
            let me = Arc::clone(&self);
            set.spawn_blocking(move || {
                me.sock_read_worker(&udp_socket, packet_tx)
            });
        }
       

        //let work_result = self.sock_read_worker(packet_tx).await;
